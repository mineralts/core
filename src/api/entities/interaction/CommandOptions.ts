...
export default class CommandOptions {
  ...
  public getBoolean (name: string): boolean | undefined {
    const numberValue = this.params?.find((param: CommandParamsResolvable) => param.name == name) as unknown as { value: boolean }
    return numberValue?.value
  }

  public getChoices<T> (name: string): T | undefined {
    const choiceValue = this.params?.find((param: CommandParamsResolvable) => param.name == name) as unknown as { value: T }
